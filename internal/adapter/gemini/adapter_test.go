...
func TestAdapter_Identity(t *testing.T) {
	...
	if a.Name() != "Gemini CLI" {
		t.Fatalf("Name() = %q, want %q", a.Name(), "Gemini CLI")
	}
	if a.Agent() != model.AgentGeminiCLI {
		t.Fatalf("Agent() = %q, want %q", a.Agent(), model.AgentGeminiCLI)
	}
...
func TestAdapter_ConfigPath(t *testing.T) {
	...
	want := filepath.Join("/home/test", ".gemini", "settings.json")
	if got != want {
...
func TestAdapter_Detect_LookPathCalledWithGemini(t *testing.T) {
	...
	if calledWith != "gemini" {
		t.Fatalf("LookPath called with %q, want %q", calledWith, "gemini")
	}
}
